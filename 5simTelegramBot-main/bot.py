# ═══════════════════════════════════════════════════════════════
# bot.py — Enterprise Customer Bot (Webhook Mode)
# ═══════════════════════════════════════════════════════════════
# Uses WEBHOOK set EXTERNALLY (via curl / Telegram API).
# Flask receives updates at / from Telegram servers.
...
def language_menu_handler(call):
    ...
    for lang in get_all_languages():
        keyboard.add(types.InlineKeyboardButton(lang['name'], callback_data=f"setlang_{lang['code']}"))
    keyboard.add(types.InlineKeyboardButton(get_text(user_id, 'navigation.back_to_main'), callback_data="back_to_main"))
    bot.edit_message_text(get_text(user_id, 'language.select_title'), call.message.chat.id, call.message.message_id, reply_markup=keyboard)
...
def handle_language_selection(call):
    ...
    if set_user_language(user_id, lang_code):
        ...
        bot.edit_message_text(get_text(user_id, 'welcome_back'), call.message.chat.id, call.message.message_id, reply_markup=main_menu_keyboard(user_id))


@bot.callback_query_handler(func=lambda call: call.data == "back_to_main")
def back_to_main(call):
    from bot.keyboards.main_keyboard import main_menu_keyboard
    bot.edit_message_text(get_text(call.from_user.id, 'welcome_back'), call.message.chat.id, call.message.message_id, reply_markup=main_menu_keyboard(call.from_user.id))
    ...
    try:
        ...
        if success:
            ...
            if new_balance is not None:
                ...
                return render_template('payment_result.html', success=True, amount=f"{int(amount):,}", ref_id=ref_id or '---', balance=f"{new_balance:,}")
...
# ═══════════════════════════════════════════════════════════════
# STARTUP — Webhook set EXTERNALLY, just run Flask
# ═══════════════════════════════════════════════════════════════
if __name__ == '__main__':
    ...
    logging.info("🌐 Flask starting on port 5000 — ready for webhook updates")
